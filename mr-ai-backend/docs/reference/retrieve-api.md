...
| `overlay_meta` | Present only in MR mode. `*_truncated` flags signal that the overlay walker hit `MR_FANOUT_*` caps. |
...
- The MR-mode embedding pass touches every overlay chunk. Tune
  `MR_FANOUT_MAX_CHUNKS` (see [Configuration](../guides/configuration.md))
  to keep retrieval latency bounded.
- `/search_vector_base` is still wired for backwards compatibility;
  expect it to be removed once external callers have migrated.
